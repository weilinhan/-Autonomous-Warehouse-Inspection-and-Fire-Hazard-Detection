...
#include <transbot_astra/KCFTrackerConfig.h>
...
public:
    ...
    int center_x;
    KCFTracker tracker;
    dynamic_reconfigure::Server<transbot_astra::KCFTrackerConfig> server;
    dynamic_reconfigure::Server<transbot_astra::KCFTrackerConfig>::CallbackType f;

    ImageConverter(ros::NodeHandle &n);

    ~ImageConverter();

    void PIDcallback(transbot_astra::KCFTrackerConfig &config, uint32_t level);
