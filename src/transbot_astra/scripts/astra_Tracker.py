...
from transbot_astra.cfg import AstraColorHSVConfig
...
class Astra_Tracker:
    def __init__(self):
        ...
        text_path = '/home/' + self.user_name + '/transbot_ws/src/transbot_astra/scripts'
        ...
        if self.VideoSwitch == False:
            ...
            img_topic = rospy.get_param("~camera", "/camera/rgb/image_raw")
            #self.sub_img = rospy.Subscriber(img_topic, CompressedImage, self.compressed_topic, queue_size=10)
            self.sub_img = rospy.Subscriber(img_topic, Image, self.image_topic, queue_size=10)
